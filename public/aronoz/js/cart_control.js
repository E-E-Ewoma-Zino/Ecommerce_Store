...
if (window.location.pathname == "/cart") {
	...
	if (!logIn) {
		...
		postCartData({}, getCartItem(), (err, res) => {
			if (err) {
				// toggle 
...
function addToCart2(cart, productID, quantity) {
	if (logIn) {
		...
		postCartData(cart, data, (err, res) => {
			if (err) {
				// toggle 
				console.error("Could not add to cart! ", err);
				cart.innerHTML = `<i class="fas fa-times text-danger"></i>`;
...
function postCartData(cart, data, callback) {
	// if (!navigator.onLine) {
...
function cartBody(items) {
	...
	items.forEach((item, index) => {
		if (item) concat += body(item.img[0].path, item.name, getCartItem()[index].quantity, item.totalNo, item.price, item._id);
		else messager({
			replace: ["success", "danger"],
			message: "Failed to load broken cart"
		});
	});
	concat += `<tr>
	<td></td>
